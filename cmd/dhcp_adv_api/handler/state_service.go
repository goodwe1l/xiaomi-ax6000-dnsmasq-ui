...
func buildStaticLeases() ([]staticLease, error) {
	hostSections, err := utils.ListSectionsByType(uciClient, appConfig.DHCP.ConfigName, "host")
	if err != nil {
		return nil, err
	}
	...
	for _, sec := range hostSections {
		...
		if hgw == "" && htag != "" {
			if value, err := utils.GetTagOptionValue(uciClient, appConfig.DHCP.ConfigName, htag, "3"); err != nil {
				return nil, err
			} else {
				hgw = value
			}
		...
		if hdns == "" && htag != "" {
			if value, err := utils.GetTagOptionValue(uciClient, appConfig.DHCP.ConfigName, htag, "6"); err != nil {
				return nil, err
			} else {
				hdns = value
			}
		...
		tagShow := ""
		if htag != "" {
			if label, err := utils.GetTagLabelFromHostTag(uciClient, appConfig.DHCP.ConfigName, htag); err != nil {
				return nil, err
			} else {
				tagShow = label
			}
...
func buildTemplates() ([]tagTemplate, error) {
	tagSections, err := utils.ListSectionsByType(uciClient, appConfig.DHCP.ConfigName, "tag")
	if err != nil {
		return nil, err
	}
	...
	for _, sec := range tagSections {
		...
		tgw, err := utils.GetSectionOptionValue(uciClient, sec, "3")
		if err != nil {
			return nil, err
		}
		tdns, err := utils.GetSectionOptionValue(uciClient, sec, "6")
		if err != nil {
			return nil, err
		}
		inUse, err := utils.TemplateInUse(uciClient, appConfig.DHCP.ConfigName, sec, tname)
		if err != nil {
			return nil, err
		}
...
func buildStatePayload() (statePayload, error) {
	...
	defaultGateway, err := utils.GetSectionOptionValue(uciClient, "lan", "3")
	if err != nil {
		return payload, err
	}
	defaultDNS, err := utils.GetSectionOptionValue(uciClient, "lan", "6")
	if err != nil {
		return payload, err
	}
	...
	staticSet, err := utils.BuildStaticMACSet(uciClient, appConfig.DHCP.ConfigName)
	if err != nil {
		return payload, err
	}
