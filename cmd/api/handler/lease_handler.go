...
func applyLeaseUpsert(r *http.Request) (string, error) {
	...
	oldSec, foundOld, err := uciutil.FindHostSecByMAC(uciClient, appConfig.DHCP.ConfigName, mac)
	if err != nil {
		return "", err
	}
	oldTag := ""
	...
	if foundOld && oldSec != hostSec {
		...
		if err := uciutil.DeleteAdvTagByName(uciClient, appConfig.DHCP.ConfigName, oldTag); err != nil {
			return "", err
		}
	}
	...
	if tag != "" {
		tSec, foundTagSec, err := uciutil.ResolveTagSec(uciClient, appConfig.DHCP.ConfigName, tag)
		if err != nil {
			return "", err
		}
		if !foundTagSec {
			...
			idx := 0
			for {
				exists, err := uciutil.SectionExists(uciClient, tSec)
				if err != nil {
					return "", err
				}
				if !exists {
...
func applyLeaseDelete(mac string) (string, error) {
	...
	sec, found, err := uciutil.FindHostSecByMAC(uciClient, appConfig.DHCP.ConfigName, mac)
	if err != nil {
		return "", err
	}
	if !found {
	...
	if err := uciutil.DeleteAdvTagByName(uciClient, appConfig.DHCP.ConfigName, htag); err != nil {
		return "", err
	}
	if err := uciClient.Commit(appConfig.DHCP.ConfigName); err != nil {
		return "", err
