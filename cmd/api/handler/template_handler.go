...
func applyTemplateUpsert(templateSec, templateTag, templateGateway, templateDNS string) (string, error) {
	...
	var tsec string
	if templateSec != "" {
		isSec, err := uciutil.IsTagSection(uciClient, templateSec)
		if err != nil {
			return "", err
		}
		if !isSec {
		...
		existingSec, found, err := uciutil.ResolveTagSec(uciClient, appConfig.DHCP.ConfigName, tname)
		if err != nil {
			return "", err
		}
...
func applyTemplateDelete(templateSec string) (string, error) {
	...
	isSec, err := uciutil.IsTagSection(uciClient, sec)
	if err != nil {
		return "", err
	}
	if !isSec {
	...
	inUse, err := uciutil.TemplateInUse(uciClient, appConfig.DHCP.ConfigName, sec, label)
	if err != nil {
		return "", err
	}
	if inUse {
